...
const AddProduct = () => {
  ...
  const handleDrop = useCallback((e) => { e.preventDefault(); setIsDragging(false); handleFileChange(e.dataTransfer.files); }, []);

  const handleSubmit = (e) => {
    e.preventDefault();
    const productData = {
      productName, description, price, category, uniqueId, image: imageFile ? imageFile.name : 'No image'
    };
  ...
  return (
    <div className="bg-gray-100 dark:bg-gray-900 min-h-screen flex items-center justify-center p-4 font-sans antialiased">
  <div className="w-full max-w-3xl bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 md:p-12 text-gray-900 dark:text-white">
    ...
    <div className="text-center md:text-left mb-10">
      ...
      <p className="mt-2 text-gray-700 dark:text-gray-300">Fill out the details below to add a new item to the catalog.</p>
    </div>
    ...
    <form onSubmit={handleSubmit} className="space-y-6 md:space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <div className="relative">
            <span className="absolute inset-y-0 left-0 pl-4 flex items-center text-gray-500 dark:text-gray-300">₹</span>
            <input
              type="number"
              value={price}
        ...
        <div className="md:col-span-2">
          <label className="block text-sm font-medium mb-2">Unique ID</label>
          <input
            type="text"
            value={uniqueId}
      ...
      <div>
        ...
        >
          <div className="space-y-2 text-center text-gray-900 dark:text-white">
            ...
            <div className="flex text-sm justify-center items-center gap-1">
              <label htmlFor="file-upload" className="relative cursor-pointer bg-white dark:bg-gray-700 rounded-md font-medium text-indigo-600 hover:text-indigo-500 px-2 py-1">
                <span>Upload a file</span>
                <input id="file-upload" type="file" className="sr-only" onChange={(e) => handleFileChange(e.target.files)} />
              </label>
        ...
        {imageFile && <p className="text-sm mt-2">File selected: <span className="font-medium">{imageFile.name}</span></p>}
      </div>

      <div className="border-t border-gray-200 dark:border-gray-600 pt-6 flex flex-col sm:flex-row sm:justify-end gap-4">
        <button type="button" className="px-6 py-3 bg-gray-100 dark:bg-gray-700 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition duration-200">
          Cancel
        </button>
        <button type="submit" className="px-8 py-3 bg-indigo-600 text-white font-semibold rounded-lg shadow-md hover:bg-indigo-700 transition duration-200">
          Add Product
        </button>
      </div>
